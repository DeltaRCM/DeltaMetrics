...
class TestComputeBoxyStratigraphyCoordinates:
    ...
    def test_returns_sc_dc_given_z(self):
        ...
        sc, dc = compute_boxy_stratigraphy_coordinates(self.elev, z=z)
    ...
    def test_returns_sc_dc_given_nz(self):
        sc, dc = compute_boxy_stratigraphy_coordinates(self.elev, nz=13)
        assert np.min(sc[:, 0]) == 0
...
class TestComputeElevationToPreservation:
    def test_1d_shorts(self):
        ...
        s2, p2 = _compute_elevation_to_preservation(np.array([1, 2]))
        s3, p3 = _compute_elevation_to_preservation(np.array([1, 2, 3]))
...
class TestComputePreservationToCube:
    def test_1d_shorts(self):
        ...
        sc15, dc15 = _compute_preservation_to_cube(np.array([5]), z)
        sc2, dc2 = _compute_preservation_to_cube(np.array([1, 2]), z)
        sc3, dc3 = _compute_preservation_to_cube(np.array([1, 2, 3]), z)
...
class TestDetermineStratCoordinates:
    ...
    def test_given_z(self):
        ...
        z = _determine_strat_coordinates(e, z=z_in)
        assert np.all(z == z_in)

    def test_given_z_scalar(self):
        e = np.array([0, 1, 1, 2, 1])
        with pytest.raises(ValueError):
            _ = _determine_strat_coordinates(e, z=0.05)

    def test_given_dz(self):
        e = np.array([0, 1, 1, 2, 1])
        z = _determine_strat_coordinates(e, dz=0.05)
        assert len(z) == 41
        assert z[-1] == 2.00
        assert z[0] == 0
        assert z[1] - z[0] == 0.05
    ...
    def test_given_dz_zero(self):
        ...
        with pytest.raises(ValueError, match=r'"dz" or "nz" cannot *.'):
            _ = _determine_strat_coordinates(e, dz=0)

    def test_given_dz_negative(self):
        e = np.array([0, 1, 1, 2, 1])
        with pytest.raises(ValueError, match=r'"dz" or "nz" cannot *.'):
            _ = _determine_strat_coordinates(e, dz=-0.5)

    def test_given_nz(self):
        e = np.array([0, 1, 1, 2, 1])
        z = _determine_strat_coordinates(e, nz=50)
        assert len(z) == 50 + 1
        assert z[-1] == 2.00
        assert z[0] == 0
    ...
    def test_given_nz_zero(self):
        e = np.array([0, 1, 1, 2, 1])
        with pytest.raises(ValueError, match=r'"dz" or "nz" cannot *.'):
            _ = _determine_strat_coordinates(e, nz=0)

    def test_given_nz_negative(self):
        e = np.array([0, 1, 1, 2, 1])
        with pytest.raises(ValueError, match=r'"dz" or "nz" cannot *.'):
            _ = _determine_strat_coordinates(e, nz=-5)

    def test_given_z_and_dz(self):
        e = np.array([0, 1, 1, 2, 1])
        z_in = np.arange(0, 10, step=0.2)
        z = _determine_strat_coordinates(e, z=z_in, dz=0.1)
        assert np.all(z == z_in)
        assert z[1] - z[0] == 0.2
    ...
    def test_given_z_and_nz(self):
        e = np.array([0, 1, 1, 2, 1])
        z_in = np.arange(0, 10, step=0.2)
        z = _determine_strat_coordinates(e, z=z_in, nz=0.1)
        assert np.all(z == z_in)
        assert z[1] - z[0] == 0.2
    ...
    def test_given_z_and_dz_and_nz(self):
        e = np.array([0, 1, 1, 2, 1])
        z_in = np.arange(0, 10, step=0.2)
        z = _determine_strat_coordinates(e, z=z_in, dz=0.1, nz=50000)
        assert np.all(z == z_in)
        assert z[1] - z[0] == 0.2

    def test_given_dz_and_nz(self):
        e = np.array([0, 1, 1, 2, 1])
        z = _determine_strat_coordinates(e, dz=0.1, nz=50000)
        assert z[0] == 0
        assert z[-1] == 2
        assert z[1] - z[0] == 0.1
...
class TestSubsidenceElevationAdjustment:
    def test_shapes_not_matching(self):
        ...
        s = np.zeros((2, 4, 2))
        with pytest.raises(ValueError):
            _adjust_elevation_by_subsidence(e, s)

    def test_1D_sigma_dist_as_float(self):
        e = np.zeros((10,))
        s = 1.0
        adj = _adjust_elevation_by_subsidence(e, s)
    ...
    def test_1d_flat(self):
        ...
        # apply function
        adj = _adjust_elevation_by_subsidence(topo, sigma_dist)
